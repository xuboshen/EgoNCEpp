...
class AllGather(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        return (
            grad_output[
                ctx.batch_size * ctx.local_rank : ctx.batch_size * (ctx.local_rank + 1)
            ],
            None,
            None,
        )
        ...
        with torch.no_grad():
            ...
            for dl_idx, dl in enumerate(self.valid_data_loader):
                for data in tqdma(dl, desc=f"Validating dl{dl_idx}"):
                    ...
                    ) and current_batch_size < (dl.batch_size or 1):
                        scattered_len = len(
                            self.model.scatter(
                                [torch.empty(current_batch_size)],
                                {},
            ...
            for dl_idx, data in enumerate(data_li):
                ...
                with torch.set_grad_enabled(True):
                    ...
                    loss = torch.mean(
                        data_state.T
                        * self.loss(
                            vid_scores.squeeze(dim=-1),
                            torch.argmax(data_label.long(), dim=1),
                        )
                    )
        ...
        with torch.no_grad():
            ...
            for dl_idx, dl in enumerate(self.valid_data_loader):
                for batch_idx, data in enumerate(tqdm.tqdm(dl)):
                    ...
                    vid_embed_all = [
                        torch.zeros_like(vid_scores) for _ in range(self.n_gpu)
                    ]
                    data_label_all = [
                        torch.zeros_like(data_label) for _ in range(self.n_gpu)
                    ]
                    data_state_all = [
                        torch.zeros_like(data_state) for _ in range(self.n_gpu)
                    ]
                    data_fps_all = [
                        torch.zeros_like(data_fps) for _ in range(self.n_gpu)
                    ]
                    data_start_all = [
                        torch.zeros_like(data_start) for _ in range(self.n_gpu)
                    ]
                    data_end_all = [
                        torch.zeros_like(data_end) for _ in range(self.n_gpu)
                    ]
                    data_pnr_all = [
                        torch.zeros_like(data_pnr) for _ in range(self.n_gpu)
                    ]
        ...
        for dl_idx in range(len(self.valid_data_loader)):
            ...
            for metric in self.metrics:
                ...
                res = metric(
                    ...
                    sc_labels_arr,
                    fps_arr,
                    parent_start_frames_arr,
                    parent_end_frames_arr,
                    parent_pnr_frames_arr,
                )
                if self.args.rank == 0:
                    self.logger.info(
                        verbose(
