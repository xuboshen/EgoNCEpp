...
class Multi_Trainer_dist_egohoi_ablation(Multi_BaseTrainer_dist):
    """
    Trainer class
    ...
    def _train_epoch(self, epoch):
        ...
        """
# sum([x.batch_size for x in self.data_loader])
        self.model.train()
        total_loss = [0] * len(self.data_loader)
        total_metrics = np.zeros(len(self.metrics))
        start = time.time()
        since = time.time()
        accumulative_iterations = 0
        index_ai = 0
        accumulation = [self.max_samples_per_epoch // 20, self.max_samples_per_epoch // 10,
                        self.max_samples_per_epoch // 5, self.max_samples_per_epoch // 2,
        ...
        for batch_idx, data_li in enumerate(zip(*self.data_loader)):
            if (batch_idx + 1) * self.total_batch_sum > self.max_samples_per_epoch:
                break
            if (batch_idx + 1) * self.total_batch_sum > accumulative_iterations: 
                val_log = self._valid_epoch(epoch)
                print(val_log)
                accumulative_iterations = accumulation[index_ai]
                index_ai += 1
            ...
            for dl_idx, data in enumerate(data_li):
                ...
                if self.config['loss']['type'] == 'Hal_SoftmaxLoss_t2v':
                    n_embeds = data['noun_vec'].to(self.device)
                    # v_embeds = data['verb_vec'].to(self.device)
