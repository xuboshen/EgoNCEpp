...
import model.egovlp_lora as lora_arch 
import utils.visualizer as module_vis
from parse_config import ConfigParser
from trainer import Multi_Trainer_dist_egohoi_ablation
...
def run():
    ...
    model = config.initialize('arch', lora_arch)
    ...
    if 'lr_scheduler' in config._config:
        lr_scheduler = CosineAnnealingWarmupRestarts(optimizer, 
            first_cycle_steps=config._config['lr_scheduler']['args']['first_cycle_steps'] // config._config['data_loader'][0]['args']['batch_size'],
            max_lr=config._config['optimizer']['args']['lr'],
            min_lr=config._config['lr_scheduler']['args']['min_lr'],
            warmup_steps=config._config['lr_scheduler']['args']['warmup_steps'] // config._config['data_loader'][0]['args']['batch_size']
            )
    ...
    trainer = Multi_Trainer_dist_egohoi_ablation(args, model, loss, metrics, optimizer,
                      config=config,
...
def init_dataloaders(config, module_data):
    ...
    if "type" in config["data_loader"] and "args" in config["data_loader"]:
        ...
        for dl_cfg in config['data_loader']:
            ...
            # dl_cfg['args'] = replace_nested_dict_item(dl_cfg['args'], 'batch_size', config['data_loader']['args'][])
            dl_cfg['args'] = replace_nested_dict_item(dl_cfg['args'], 'neg_num', 10)
