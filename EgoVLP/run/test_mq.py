...
def run():
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm.tqdm(data_loader)):
            ...
            # pdb.set_trace()
            if os.path.exists(
                os.path.join(args.save_feats, data["meta"]["clip_uid"][0] + ".pt")
            ):
            ...
            for j in range(times):
                ...
                outs[start:end, :] = model.compute_video(
                    data["video"][
                        start:end,
                    ]
                )
            ...
            torch.save(
                outs, os.path.join(args.save_feats, data["meta"]["clip_uid"][0] + ".pt")
            )
...
if __name__ == "__main__":
    ...
    args.add_argument(
        "--save_feats",
        default="dataset/ego4d/benchmark_splits/mq/egovlp",
