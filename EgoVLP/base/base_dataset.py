...
# mainly for egocentric pretraining
def read_frames_decord_start_end_fastchunk(root, vid, second, end_second=None, chunk_len=300, fps=30, clip_length=32, jitter=False):
    if chunk_len == -1:
    ...
    if max(frame_ids) < len(vr):
        ...
        try:
            ...
            frames_part2 = vr2.get_batch(frame_ids_part2).numpy()
    ...
    frames = [torch.tensor(frame, dtype=torch.float32) / 255 for frame in frames]
...
video_reader = {
    ...
    'decord_start_end_fastchunk': read_frames_decord_start_end_fastchunk,
}
