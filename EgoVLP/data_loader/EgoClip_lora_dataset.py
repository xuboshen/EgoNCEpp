...
class EgoClip_lora(EgoClip_EgoMCQ):
    ...
    def _get_train_item(self, item):
        ...
        if self.neg_param:
            ...
            try:
                caption_neg = sample['clip_text_neg'].split(',')
            except:
                sample_temp = self.metadata.iloc[item + 1]
                caption_neg = sample_temp['clip_text_neg'].split(',')
        ...
        if self.neg_param:
            if self.video_params['egonce'] == True:
    ...
    def _get_val_item(self, item):
        ...
        for id, option in enumerate(sampleOptions):
            ...
            imgs = self._get_video_frames(video_fp, start_sec, end_sec, clip_length=self.video_params['num_frames'])
