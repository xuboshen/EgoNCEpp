...
class EgoClip_EgoMCQ(TextVideoDataset):
    ...
    def _get_train_item(self, item):
        ...
        if self.neg_param:
            ...
            final_neg = self._get_video_frames(video_fp_neg, start_sec_neg, end_sec_neg, clip_length=self.video_params['num_frames'], jitter=True)
    ...
    def _get_val_item(self, item):
        ...
        for id, option in enumerate(sampleOptions):
            ...
            imgs = self._get_video_frames(video_fp, start_sec, end_sec, clip_length=self.video_params['num_frames'], jitter=False)
            videoOptions[id] = imgs

        type =  itemMCQ['types']    # 1 for inter; 2 for intra
        data = {'video': videoOptions, 'text': textQuery, 'text_ops':textOptions, 'correct': answerIndex, 'type': type, 'meta': meta_arr}
        return data
