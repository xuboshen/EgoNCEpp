...
class EgoClip_HOI(EgoClip_EgoMCQ):
    ...
    def _get_val_item(self, item):
        ...
        try:
            verb_caption_neg = self._get_caption_neg_val(
                sample, "action_antonym_clip_text"
            )
            noun_caption_neg = self._get_caption_neg_val(
                sample, "action_antonym_clip_text"
            )
        except:
            sample_temp = self.metadata.iloc[item + 1]
            verb_caption_neg = self._get_caption_neg_val(
                sample_temp, "action_antonym_clip_text"
            )
            noun_caption_neg = self._get_caption_neg_val(
                sample_temp, "action_antonym_clip_text"
            )
