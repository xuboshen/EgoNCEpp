...
class GPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        if (
            ...
            and layer_idx % config.add_cross_attention_freq == 0
        ):
            ...
            self.ln_2_crossattention = nn.LayerNorm(
                hidden_size, eps=config.layer_norm_epsilon
            )
    ...
    def forward(
        ...
        if (
            ...
            and self.attn.layer_idx % self.add_cross_attention_freq == 0
        ):
            ...
            if hasattr(self, "alpha_dense"):
                feed_forward_hidden_states = (
                    torch.tanh(self.alpha_dense) * feed_forward_hidden_states
                )
            # residual connection
