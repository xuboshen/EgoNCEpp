...
class VCLM_HF(nn.Module):
    ...
    def encode_image(self, image, use_checkpoint=False):
        if isinstance(self.visual, VisionTransformer):
            ...
            bb, tt, _, _, _ = image.shape
            x = self.visual.forward_features(
    ...
    def forward(self, image, text, mask=None, use_checkpoint=False, norm_embed=False):
        ...
        output_decoder = self.text_decoder(
            text.contiguous(), encoder_hidden_states=image_tokens
        )
        text_tokens_logits = output_decoder.logits
        text_tokens_logits = rearrange(text_tokens_logits, "b n c -> b c n")

        return {"text_tokens_logits": text_tokens_logits, "labels": labels}
    ...
    def generate(
        ...
        generated_text_ids = torch.LongTensor(
            [[tokenizer.bos_token_id]] * image_tokens.shape[0]
        ).to(device)
        ...
        nlls, num_tokens = torch.zeros(image_tokens.shape[0]).to(device), torch.zeros(
        ...
        with torch.no_grad():
            for i in range(max_text_length - 1):
                output_decoder = self.text_decoder(
                    condition_text_ids, encoder_hidden_states=image_tokens
                )
                ...
                if target is not None:
                    ...
                    nll = torch.special.entr(F.softmax(next_token_logits, dim=1)).sum(
                        dim=1
                    )
                ...
                is_reach_eos = is_reach_eos | (
                    next_token[:, 0] == tokenizer.eos_token_id
                )
                ...
                if teacher_forcing:
                    ...
                    condition_text_ids = torch.cat(
                        (generated_text_ids, next_token), dim=1
                    )
    ...
    def beam_sample(
        ...
        expanded_return_idx = (
            ...
            .view(-1, 1)
            .repeat(1, num_beams * num_return_sequences)
            .view(-1)
            .to(device)
        )
        ...
        with torch.no_grad():
            for i in range(max_text_length - 1):
                output_decoder = self.text_decoder(
                    input_ids,
                    encoder_hidden_states=image_tokens.repeat_interleave(
                        num_beams * num_return_sequences, dim=0
                    ),
                )
                decoded_token_logits = output_decoder.logits
                next_token_logits = decoded_token_logits[:, -1, :]
    ...
    def _get_logits_warper(
        ...
        typical_p = typical_p if typical_p is not None else 1.0
