...
def convert_weights(model: nn.Module):
    ...
    def _convert_weights_to_fp16(tmp_l):
        if isinstance(tmp_l, (nn.Conv1d, nn.Conv2d, nn.Linear)):
            tmp_l.weight.data = tmp_l.weight.data.half()
            if tmp_l.bias is not None:
                tmp_l.bias.data = tmp_l.bias.data.half()

        if isinstance(tmp_l, nn.MultiheadAttention):
            for attr in [
                *[f"{s}_proj_weight" for s in ["in", "q", "k", "v"]],
                ...
                tensor = getattr(tmp_l, attr)
                if tensor is not None:
                    tensor.data = tensor.data.half()

        for name in ["text_projection", "proj"]:
            if hasattr(tmp_l, name):
                attr = getattr(tmp_l, name)
                if attr is not None:
