...
class VideoCaptionDatasetBase(torch.utils.data.Dataset):
    ...
    def get_raw_item(self, i, is_training=True, num_clips=1, clip_length=32, clip_stride=2, sparse_sample=False,
        ...
        if self.dataset == 'ego4d':
            ...
            verb_choice = str(itemHOI['verb_neg']).split(', ')
            noun_choice = str(itemHOI['noun_neg']).split(', ')
...
class VideoCaptionDatasetHOI(VideoCaptionDatasetBase):
    ...
    def __getitem__(self, i):
        ...
        # print(textQuery, verb_choice, noun_choice)
        # apply transformation
        if self.transform is not None:
            frames = self.transform(frames)
            # frames_options = [self.transform(frames) for frames in frames_options]
        ...
        if self.tokenizer is not None:
            textQuery = self.tokenizer(textQuery)
            verb_choice = self.tokenizer(verb_choice)
            noun_choice = self.tokenizer(noun_choice)
            # narration_options = self.tokenizer(narration_options)
            # if isinstance(textQuery, tuple):
            #     textQuery, mask_query = textQuery
            #     narration_options, mask_options = narration_options
        ...
        # print(textQuery.shape, frames.shape, verb_choice.shape, noun_choice.shape)
        return textQuery, frames, verb_choice, noun_choice
...
class VideoCaptionDatasetEgohoi(VideoCaptionDatasetBase):
    ...
    def __getitem__(self, i):
        ...
        if self.tokenizer is not None:
            textQuery = self.tokenizer(textQuery)
            txt_neg = self.tokenizer(txt_neg)
            # narration_options = self.tokenizer(narration_options)
            # if isinstance(textQuery, tuple):
            #     textQuery, mask_query = textQuery
            #     narration_options, mask_options = narration_options
        ...
        # print(textQuery.shape, frames.shape, verb_choice.shape, noun_choice.shape)
        return textQuery, frames, txt_neg, noun_vec, verb_vec
