...
def generate_label_map(dataset):
    if dataset == "ek100_cls":
        ...
        for f in [
            "/fs/fast/base_path/data/EK100_256p/epic-kitchens-100-annotations/EPIC_100_train.csv",
            "/fs/fast/base_path/data/EK100_256p/epic-kitchens-100-annotations/validation_close_ov.csv",
        ]:
        ...
        labels = []
        with open(
            "/fs/fast/base_path/annotations/CharadesEgo/Charades_v1_classes.txt"
        ) as f:
        ...
        labels = []
        with open("/fs/fast/base_path/data/egtea/action_idx.txt") as f:
            for row in f:
                row = row.strip()
