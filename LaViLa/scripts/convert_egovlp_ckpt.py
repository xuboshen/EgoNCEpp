...
def get_args_parser():
    parser = argparse.ArgumentParser(
        description="Convert EgoVLP checkpoint", add_help=False
    )
...
def main(args):
    ...
    for k in input_ckpt:
        if k.startswith("module.video_model"):
            output_ckpt[k.replace("module.video_model", "module.visual")] = input_ckpt[
                k
            ]
        elif k.startswith("module.text_model"):
            output_ckpt[k.replace("module.text_model", "module.textual")] = input_ckpt[
                k
            ]
        elif k.startswith("module.txt_proj"):
            output_ckpt[
                k.replace("module.txt_proj", "module.text_projection")
            ] = input_ckpt[k]
        elif k.startswith("module.vid_proj"):
            output_ckpt[
                k.replace("module.vid_proj", "module.image_projection")
            ] = input_ckpt[k]
        else:
            print(k)
            raise ValueError
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Convert EgoVLP checkpoint", parents=[get_args_parser()]
    )
