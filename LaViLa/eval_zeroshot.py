...
def get_args_parser():
    parser = argparse.ArgumentParser(
        description="LAVILA 0-shot evaluations", add_help=False
    )
    parser.add_argument(
        "--dataset",
        default="ek100_mir",
    ...
    parser.add_argument(
        "--num-crops", default=1, type=int, help="number of crops in transforms"
    )
    parser.add_argument(
        "--num-clips",
        default=1,
        type=int,
        help="number of clips (for untrimmed videos, eg. Charades)",
    )
    ...
    parser.add_argument(
        "--cls-use-template",
        action="store_true",
        help="use prompt in 0-shot classification",
    )
...
def main(args):
    ...
    try:
        ...
        print(E)
        old_args.model = "CLIP_OPENAI_TIMESFORMER_BASE"
    # import pdb; pdb.set_trace()
    model = getattr(models, old_args.model)(
        text_use_cls_token=False
        if "use_cls_token" not in old_args
        else old_args.use_cls_token,
        project_embed_dim=256
        if "project_embed_dim" not in old_args
        else old_args.project_embed_dim,
        gated_xattn=False if "gated_xattn" not in old_args else old_args.gated_xattn,
        timesformer_gated_xattn=False
        if "timesformer_gated_xattn" not in old_args
        else old_args.timesformer_gated_xattn,
        timesformer_freeze_space=False
        if "timesformer_freeze_space" not in old_args
        else old_args.timesformer_freeze_space,
        freeze_lm_vclm=False
        if "freeze_lm_vclm" not in old_args
        else old_args.freeze_lm_vclm,
        freeze_visual_vclm=False
        if "freeze_visual_vclm" not in old_args
        else old_args.freeze_visual_vclm,
    ...
    print(
        "=> loaded resume checkpoint '{}' (epoch {}, best_metric = {})".format(
            args.resume, ckpt["epoch"], ckpt["best_acc1"]
        )
    )
    ...
    if args.dataset in ["ek100_cls", "charades_ego", "egtea"]:
        preds, targets = validate_zeroshot(
    ...
    if args.dataset == "ek100_mir":
        ...
        similarity_matrix = get_similarity_matrix(
            val_loader, model, print_freq=args.print_freq, use_half=args.use_half
        )
        similarity_matrix = (similarity_matrix + 1) / 2
        video_id = pd.read_csv(args.metadata_val).values[:, 0]
        text_id = pd.read_csv(
            args.metadata_val.replace("test.csv", "test_sentence.csv")
        ).values[:, 0]
    ...
    if args.dataset == "ego4d_mcq_v2t":
        val_dataset = datasets.VideoCaptionDatasetMCQ_v2t(
            args.dataset,
            args.root,
            args.metadata_val,
            transform=val_transform,
        ...
        validate_mcq_v2t(val_loader, model, use_half=args.use_half)
    if args.dataset == "ego4d_hoi":  # TODO
        val_dataset = datasets.VideoCaptionDatasetHOI(
            args.dataset,
            args.root,
            args.metadata_val,
            transform=val_transform,
        ...
        validate_hoi(val_loader, model, use_half=args.use_half)
    ...
    with torch.no_grad():
        ...
        for label in labels:
            ...
            if masks is not None:
                class_embeddings = dist_utils.get_model(model).encode_text(
                    texts, attention_mask=masks
                )
            else:
        ...
        for i, (images, target) in enumerate(val_loader):
            if i % args.print_freq == 0:
                print(
                    "finish batch {}/{} in {} sec".format(
                        i, len(val_loader), time.time() - end_time
                    )
                )
                end_time = time.time()
...
def get_similarity_matrix(val_loader, model, print_freq=100, use_half=False):
    ...
    with torch.no_grad():
        ...
        for i, inputs in enumerate(val_loader):
            ...
            if masks is not None:
                text_features = dist_utils.get_model(model).encode_text(
                    texts, attention_mask=masks
                )
            else:
        ...
        if is_multiple_narrations:
            similarity_matrix = similarity_matrix.reshape(
                all_video_embed.shape[0], all_video_embed.shape[0], -1
            )
...
def validate_mcq_v2t(val_loader, model, use_half=False):
    model.eval()
    if use_half:
        model.half()
    with torch.no_grad():
        ...
        for i, inputs in enumerate(val_loader):
            if i % args.print_freq == 0:
                ...
                end_time = time.time()
            # texts_query = inputs[0].cuda(non_blocking=True)
            video_query = inputs[-1].cuda(non_blocking=True)
            # frames_options = inputs[1].cuda(non_blocking=True)
            texts_options = inputs[2].cuda(non_blocking=True)
            if use_half:
                # frames_options = frames_options.half()
                video_query = video_query.half()
            answer = inputs[3]
            q_type = inputs[4]
            if len(inputs) == 8:
                # masks_query = inputs[5].cuda(non_blocking=True)
                masks_options = inputs[6].cuda(non_blocking=True)
            else:
                # masks_query = None
                pass

            batch_size = video_query.shape[0]

            # frames_options = frames_options.view(-1, *frames_options.shape[2:])
            image_features = dist_utils.get_model(model).encode_image(video_query)
            # image_features = image_features.view(batch_size, -1, *image_features.shape[1:])

            texts_options = texts_options.view(-1, *texts_options.shape[2:])
            if len(inputs) == 8 and masks_options is not None:
                options_features = dist_utils.get_model(model).encode_text(
                    texts_options, attention_mask=masks_options
                )
            else:
                options_features = dist_utils.get_model(model).encode_text(
                    texts_options
                )
            options_features = options_features.view(
                batch_size, -1, *options_features.shape[1:]
            )
            ...
            for j in range(batch_size):
                similarity_matrix = torch.matmul(
                    image_features[j], options_features[j].T
                )
                similarity_matrix = similarity_matrix.cpu().detach()
                all_preds.append(similarity_matrix)
        all_preds = torch.stack(all_preds)
        all_gts = torch.cat(all_gts)
        all_types = torch.cat(all_types)
        metrics = egomcqv2t_accuracy_metrics(all_preds, all_gts, all_types)
        print(metrics)
...
def validate_hoi(val_loader, model, use_half=False):
    ...
    with torch.no_grad():
        ...
        for i, inputs in enumerate(val_loader):
            ...
            frames_options = (
                frames  # frames_options.view(-1, *frames_options.shape[2:])
            )
            image_features = dist_utils.get_model(model).encode_image(frames_options)
            # image_features = image_features.view(batch_size, -1, *image_features.shape[1:])
            ...
            if masks_query is not None:
                ...
                verb_features = dist_utils.get_model(model).encode_text(
                    verb_choice.view(-1, verb_choice.shape[-1])
                )
                verb_features = verb_features.view(
                    batch_size, -1, verb_features.shape[-1]
                )
                noun_features = dist_utils.get_model(model).encode_text(
                    noun_choice.view(-1, noun_choice.shape[-1])
                )
                noun_features = noun_features.view(
                    batch_size, -1, noun_features.shape[-1]
                )
            ...
            for j in range(batch_size):
                query_sim = (
                    torch.matmul(image_features[j], query_features[j].T)
                    .cpu()
                    .detach()
                    .unsqueeze(0)
                )
                verb_sim = (
                    torch.matmul(image_features[j], verb_features[j].T).cpu().detach()
                )
                noun_sim = (
                    torch.matmul(image_features[j], noun_features[j].T).cpu().detach()
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "lavila 0-shot evaluations", parents=[get_args_parser()]
    )
