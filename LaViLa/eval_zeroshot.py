...
from lavila.utils.evaluation_egomcq import egomcq_accuracy_metrics, egohoi_accuracy_metrics, egomcqv2t_accuracy_metrics
...
def get_args_parser():
    ...
    parser.add_argument('--dataset', default='ek100_mir', type=str,
                        choices=['ek100_cls', 'ek100_mir', 'charades_ego', 'egtea', 'ego4d_mcq', 'ego4d_hoi', 'ego4d_mcq_v2t'])
...
def main(args):
    ...
    model = getattr(models, old_args.model)(
        text_use_cls_token=False if 'use_cls_token' not in old_args else old_args.use_cls_token,
        project_embed_dim=256 if 'project_embed_dim' not in old_args else old_args.project_embed_dim,
    ...
    with torch.no_grad():
        ...
        for i, inputs in enumerate(val_loader):
            ...
            if len(inputs) == 8 and masks_options is not None:
                options_features = dist_utils.get_model(model).encode_text(texts_options, attention_mask=masks_options)
            else:
                options_features = dist_utils.get_model(model).encode_text(texts_options)
            options_features = options_features.view(batch_size, -1, *options_features.shape[1:])
            

            all_gts.append(answer)
            all_types.append(q_type)
            for j in range(batch_size):
                similarity_matrix = torch.matmul(image_features[j], options_features[j].T)
                similarity_matrix = similarity_matrix.cpu().detach()
                all_preds.append(similarity_matrix)
        all_preds = torch.stack(all_preds)
        all_gts = torch.cat(all_gts)
