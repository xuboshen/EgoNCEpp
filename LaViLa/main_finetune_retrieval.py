...
def main(args):
    ...
    if args.dataset == "ek100_mir":
        ...
        criterion = loss.EgoCLIP_HOILoss(temperature=0.05, noun=True)
    ...
    if args.dataset == "ek100_mir":
        ...
        train_dataset = datasets.VideoCaptionDatasetEgohoi(
            args.dataset,
            args.root,
            args.metadata,
            transform=train_transform,
    ...
    if args.evaluate:
        if args.dataset == "ek100_mir":
            ...
            _ = validate_hoi(val_loader, model)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.dataset == "ek100_mir":
            ...
            val_stats = validate_hoi(val_loader, model)
...
def train(train_loader, model, criterion, optimizer, scaler, epoch, lr_schedule, args):
    ...
    for data_iter, inputs in enumerate(train_loader):
        ...
        if (
            data_iter + 1
        ) * torch.distributed.get_world_size() * args.batch_size > args.max_samples:
            break
        ...
        with amp.autocast(enabled=not args.disable_amp):
            ...
            if args.dataset == "ek100_mir":
                ...
                image_features = model(image=frames, text=None, norm_embed=True)[
                    "image_embed"
                ]
                pos_txt = model(image=None, text=texts_query, norm_embed=True)[
                    "text_embed"
                ]
                neg_txt = model(
                    image=None,
                    text=txt_neg.view(-1, txt_neg.shape[-1]),
                    norm_embed=True,
                )["text_embed"]
...
def validate_mir(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        for i, inputs in enumerate(val_loader):
            ...
            # compute output
            outputs = model(
                *inputs, use_checkpoint=args.use_checkpoint, norm_embed=args.norm_embed
            )
            loss_dict = criterion(outputs, weight=relevancies)
