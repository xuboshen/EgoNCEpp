...
def get_args_parser():
    ...
    # Data
    parser.add_argument(
        "--dataset", default="ek100_cls", type=str, choices=["ek100_cls", "egtea"]
    )
    parser.add_argument(
        "--root",
    ...
    parser.add_argument(
        "--num-crops", default=1, type=int, help="number of crops in transforms for val"
    )
    parser.add_argument(
        "--num-clips", default=1, type=int, help="number of clips for val"
    )
    ...
    parser.add_argument(
        ...
        default=0.5,
        type=float,
        help="dropout ratio for the last linear layer",
    )
    parser.add_argument(
        "--num-classes",
        default=3806,
        nargs="+",
        type=int,
        help="number of classes for the last linear layer",
    )
    parser.add_argument("--use-vn-classifier", action="store_true")
    parser.add_argument(
        "--use-half", action="store_true", help="use half precision at inference"
    )
    # Training
    ...
    parser.add_argument(
        "--lr-multiplier-on-backbone",
        default=0.1,
        type=float,
        help="lr multiplier for the backbone",
    )
    ...
    parser.add_argument(
        "--label-smoothing", default=0.1, type=float, help="label smoothing"
    )
...
def main(args):
    ...
    if args.use_vn_classifier:
        model = models.VideoClassifierMultiHead(
            model.visual, dropout=args.dropout_ratio, num_classes_list=args.num_classes
        )
    else:
        assert len(args.num_classes) == 1
        model = models.VideoClassifier(
            model.visual, dropout=args.dropout_ratio, num_classes=args.num_classes[0]
        )
    ...
    optim_params = [
        {
            ...
            "lr": args.lr * args.lr_multiplier_on_backbone,
        },
        {
            "params": p_non_wd,
            "weight_decay": 0,
    ...
    if args.use_zero:
        optimizer = ZeroRedundancyOptimizer(
            optim_params,
            optimizer_class=torch.optim.SGD if args.use_sgd else torch.optim.AdamW,
            lr=args.lr,
            betas=args.betas,
            eps=args.eps,
            weight_decay=args.wd,
        ...
        if args.use_sgd:
            optimizer = torch.optim.SGD(
                optim_params, lr=args.lr, momentum=args.betas[0], weight_decay=args.wd
            )
        else:
    ...
    if old_args.model.endswith("DISTILBERT_BASE"):
        ...
        print(
            "Using SimpleTokenizer because of model '{}'. "
    ...
    for data_iter, (images, target) in enumerate(train_loader):
        ...
        with amp.autocast(enabled=not args.disable_amp):
            ...
            if isinstance(output, list):
                assert len(output) == 3
                target_to_verb = torch.tensor(
                    [args.mapping_act2v[a] for a in target.tolist()]
                ).cuda(args.gpu, non_blocking=True)
                loss = criterion(output[0], target_to_verb)
                target_to_noun = torch.tensor(
                    [args.mapping_act2n[a] for a in target.tolist()]
                ).cuda(args.gpu, non_blocking=True)
        ...
        if args.clip_grad_value is not None:
            ...
            if args.clip_grad_type == "norm":
                torch.nn.utils.clip_grad_norm_(
                    model.parameters(), args.clip_grad_value, norm_type=2.0
                )
            elif args.clip_grad_type == "value":
                torch.nn.utils.clip_grad_value_(
                    model.parameters(), args.clip_grad_value
                )
            else:
        ...
        if isinstance(output, list):
            ...
            if args.dataset == "ek100_cls":
                ...
                verb_scores = torch.tensor(
                    marginalize(output.detach().cpu().numpy(), vi)
                ).cuda(args.gpu, non_blocking=True)
                noun_scores = torch.tensor(
                    marginalize(output.detach().cpu().numpy(), ni)
                ).cuda(args.gpu, non_blocking=True)
                target_to_verb = torch.tensor(
                    [args.mapping_act2v[a] for a in target.tolist()]
                ).cuda(args.gpu, non_blocking=True)
                ...
                top1_verb.update(0.0, images.size(0))
                top1_noun.update(0.0, images.size(0))
...
def validate(val_loader, model, args):
    ...
    if args.dataset == "ek100_cls":
        print(
            "EK100 * Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f}".format(
                top1=top1, top5=top5
            )
        )
    else:
        print(
            "EGTEA * Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f}".format(
                top1=top1, top5=top5
            )
        )
    ...
    if dist_utils.is_main_process():
        with open(os.path.join(args.output_dir, "log.txt"), "a") as f:
            f.write(
                json.dumps({"acc1": top1.avg, "acc5": top5.avg, "mean_acc": mean_acc})
                + "\n"
            )
...
def validate_multihead(val_loader, model, args):
    ...
    top1_noun = AverageMeter("Noun Acc@1", ":6.2f")
    progress = ProgressMeter(
        len(val_loader), [batch_time, top1, top5, top1_verb, top1_noun], prefix="Test: "
    )
    ...
    print(
        "EK100 * Action Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f}".format(
            top1=top1, top5=top5
        )
    )
    return {
