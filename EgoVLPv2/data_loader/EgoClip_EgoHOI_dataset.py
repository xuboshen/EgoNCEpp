...
class EgoClip_HOI(EgoClip_EgoMCQ):
    ...
    def _get_train_item(self, item):
        ...
        if self.neg_param:
            if self.neg_param == 1:
                ...
                try:
                    ...
                    noun_caption_neg = self._get_caption_neg(sample_temp, "noun_neg")
                caption_neg = ",".join(
                    verb_caption_neg.split(",") + noun_caption_neg.split(",")
                )
        ...
        if self.neg_param:
            if self.video_params["egonce"] is True or self.neg_param in [4, 5]:
                return {
                    "video": final,
                    "text": caption,
    ...
    def _get_val_item(self, item):
        ...
        if self.neg_param:
            ...
            try:
                verb_caption_neg = self._get_caption_neg_val(sample, "verb_neg")
                noun_caption_neg = self._get_caption_neg_val(sample, "noun_neg")
            except:
                sample_temp = self.metadata.iloc[item + 1]
                verb_caption_neg = self._get_caption_neg_val(sample_temp, "verb_neg")
                noun_caption_neg = self._get_caption_neg_val(sample_temp, "noun_neg")
        ...
        return {
            "video": final,
            "text": caption,
            "verb_neg": verb_caption_neg,
            "noun_neg": noun_caption_neg,
            "meta": meta_arr,
        }
