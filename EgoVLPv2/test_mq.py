...
def run():
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm.tqdm(data_loader)):
            if os.path.exists(
                os.path.join(args.save_dir, data["meta"]["clip_uid"][0] + ".pt")
            ):
            ...
            torch.save(
                outs, os.path.join(args.save_dir, data["meta"]["clip_uid"][0] + ".pt")
            )
...
if __name__ == "__main__":
    ...
    args.add_argument(
        "-r", "--resume", help="path to latest checkpoint (default: None)"
    )
    args.add_argument(
        "-d",
        "--device",
