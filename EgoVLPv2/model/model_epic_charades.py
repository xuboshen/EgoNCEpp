...
class FrozenInTime(BaseModel):
    def __init__(
        ...
        try:
            self.relevancy_mat_train_path = os.path.join(
                self.config["data_loader"]["args"]["meta_dir"],
                "relevancy",
        ...
        if self.text_params["model"].startswith("roberta"):
            self.text_model = RobertaModel.from_pretrained(
                "/fs/fast/base_path/code/EgoVLPv2/pretrained/roberta-base"
            )
