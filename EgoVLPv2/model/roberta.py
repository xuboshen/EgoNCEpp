...
def apply_chunking_to_forward(
    ...
    *input_tensors,
    ...
    if chunk_size > 0:
        ...
        output_chunks = tuple(
            forward_fn(*input_tensors_chunk, last_norm=last_norm)
...
class RobertaLayer(nn.Module):
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            attention_output = (
                self.alpha_t2i * cross_attention_outputs[0] + attention_output
            )
    ...
    def feed_forward_chunk(self, attention_output, last_norm=True):
        ...
        layer_output = self.output(
            intermediate_output, attention_output, last_norm=last_norm
        )
        return layer_output
...
class RobertaEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [
                RobertaLayer(config, layer_index=layer_i)
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            raise ValueError(
                "You cannot secify both input_ids and inputs_embeds at the same time"
            )
