...
from model import heads, roberta, video_transformer
...
class FrozenInTime(BaseModel):
    def __init__(
        ...
        if self.text_params["model"].startswith("roberta"):
            self.text_model = RobertaModel.from_pretrained("./pretrained/roberta-base")
        ...
        if "MLM" in self.task_names or "ITM" in self.task_names:
            ...
            roberta.NUM_FUSE_BLOCK = (
                self.video_model.NUM_FUSE_BLOCK
            ) = self.num_fuse_block
    ...
    def forward(
        ...
        if "EgoNCE" in task_names:
            ...
            pos_text = allgather(pos_text, n_gpu, args)
            neg_text = text_embeds[video_embeds.shape[0] :]
            neg_text = allgather(neg_text, n_gpu, args)
            ...
            if config["loss"]["type"] == "EgoNCE_hal":
                ...
                loss = loss_egonce(output, output_hal, sim_v, sim_n)
            elif config["loss"]["type"] == "Hal_SoftmaxLoss_t2v":
                n_embeds = allgather(n_embeds, n_gpu, args)
                sim_n = sim_matrix(n_embeds, n_embeds)
                loss = loss_egonce(output, output_hal, sim_n)
            else:
                loss = loss_egonce(output, output_hal)
