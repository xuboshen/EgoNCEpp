        ...
        with torch.no_grad():
            for dl_idx, dl in enumerate(self.valid_data_loader):
                ...
                for batch_idx, data in enumerate(tqdm(dl)):
                    ...
                    verb_text = {
                        key: val.cuda(gpu, non_blocking=True)
                        for key, val in verb_text.items()
                    }
                    noun_text = {
                        key: val.cuda(gpu, non_blocking=True)
                        for key, val in noun_text.items()
                    }
                    data["text"] = verb_text
                    verb_ret = self.model.module.infer(
                        data, return_embeds=True, task_names="EgoNCE", ret={}
                    )
                    data["text"] = noun_text
                    noun_ret = self.model.module.infer(
                        data, return_embeds=True, task_names="EgoNCE", ret={}
                    )
                    verb_text_embed, noun_text_embed, vid_embed = (
                        verb_ret["text_embeds"],
                        noun_ret["text_embeds"],
                        verb_ret["video_embeds"],
                    )
