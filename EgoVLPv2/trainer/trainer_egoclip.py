...
class Multi_Trainer_dist(Multi_BaseTrainer_dist):
    ...
    def __init__(
        ...
        super().__init__(
            args, model, loss, metrics, optimizer, scheduler, gpu, config, writer
        )
        self.config = config
        self.args = args
        ...
        for batch_idx, data_li in enumerate(zip(*self.data_loader)):
            ...
            for dl_idx, data in enumerate(data_li):
                ...
                if "MLM" in self.args.task_names:
                    ...
                    data["text_mlm_ids"] = data["text_mlm_ids"].cuda(
                        gpu, non_blocking=True
                    )
                    data["text_mlm_labels"] = data["text_mlm_labels"].cuda(
                        gpu, non_blocking=True
                    )

                data["text"] = {
                    key: val.cuda(gpu, non_blocking=True)
    ...
    def _valid_epoch(self, epoch, gpu):
        ...
        with torch.no_grad():
            for dl_idx, dl in enumerate(self.valid_data_loader):
                ...
                for batch_idx, data in enumerate(tqdm(dl)):
                    ...
                    ret = self.model.module.infer(
                        data, return_embeds=True, task_names="EgoNCE", ret={}
                    )

                    data["text"]["input_ids"] = torch.repeat_interleave(
                        data["text"]["input_ids"], b2, dim=0
                    )
                    data["text"]["attention_mask"] = torch.repeat_interleave(
                        data["text"]["attention_mask"], b2, dim=0
                    )

                    ret = self.model.module.infer(
                        data, return_embeds=True, task_names="ITM", ret=ret
                    )

                    data_gt = (
                        data["correct"][0].cuda(gpu, non_blocking=True).unsqueeze(0)
                    )

                    ret["text_embeds"] = ret["text_embeds"].reshape(
                        b1, 1, ret["text_embeds"].shape[-1]
                    )
                    ret["video_embeds"] = ret["video_embeds"].reshape(
                        b1, b2, ret["video_embeds"].shape[-1]
                    )
                    data_pred_vtc = sim_matrix_batch_val(
                        ret["text_embeds"], ret["video_embeds"]
                    ).squeeze(1)
                    data_pred_vtm = (
                        F.softmax(ret["cross_attn_itm_logits"], dim=1)[:, 1:]
                        .t()
                        .reshape(1, b1, b2)[0]
                        .contiguous()
                    )
                    ...
                    data_type = (
                        data["type"][0].cuda(gpu, non_blocking=True).unsqueeze(0)
                    )
                    ...
                    data_pred_all_vtc = [
                        torch.zeros_like(data_pred_vtc) for _ in range(self.n_gpu)
                    ]
                    torch.distributed.all_gather(data_pred_all_vtc, data_pred_vtc)
                    data_pred_all_vtc = torch.cat(data_pred_all_vtc, dim=0)

                    data_pred_all_vtm = [
                        torch.zeros_like(data_pred_vtm) for _ in range(self.n_gpu)
                    ]
        ...
        for dl_idx in range(len(self.valid_data_loader)):
            ...
            pred_arr_cat = {
                "egomcq_accuracy_metrics_ensemble": torch.cat(
                    pred_arr_ensemble[dl_idx]
                ),
                "egomcq_accuracy_metrics_vtm": torch.cat(pred_arr_vtm[dl_idx]),
            }
