...
class Multi_Trainer_dist_Charades(Multi_BaseTrainer_dist):
    ...
    def __init__(
        ...
        self.args.model_name = self.config["arch"]["args"]["load_checkpoint"].split(
            "/"
        )[-1]
    ...
    def _train_epoch(self, epoch, scaler, gpu):
        ...
        if dist.get_rank() == 0:
            ...
            stats_file = open(
                ...
                / str(self.args.model_name + "_" + "stats_vtc.txt"),
                "a",
                buffering=1,
            )
            print(" ".join(sys.argv))
    ...
    def _valid_epoch(self, epoch, gpu):
        ...
        cls_arr = []
        with open(
            "/fs/fast/base_path/annotations/CharadesEgo/Charades_v1_classes.txt", "r"
        ) as charades:
        ...
        with torch.no_grad():
            ...
            data_cls = {
                key: val.cuda(gpu, non_blocking=True) for key, val in data_cls.items()
            }
            ...
            dict_cls = {
                ...
                "video": torch.Tensor(
                    1,
                    self.config["data_loader"][0]["args"]["video_params"]["num_frames"],
                    3,
                    224,
                    224,
                ).cuda(gpu, non_blocking=True),
            }  # 'video' can be random here with proper shape
            ret = self.model.module.infer(
                dict_cls, return_embeds=True, task_names="Dual", ret={}
            )
            ...
            for dl_idx, dl in enumerate(self.valid_data_loader):
                for batch_idx, data in enumerate(tqdm(dl)):
                    ...
                    ret = self.model.module.infer(
                        data, return_embeds=True, task_names="Dual", ret={}
                    )
...
def verbose(epoch, metrics, mode, args, name="TEST"):
    ...
    if dist.get_rank() == 0:
        ...
        stats_file = open(
            Path(args.save_dir) / str(args.model_name + "_" + "stats_vtc.txt"),
            "a",
            buffering=1,
        )
        print(" ".join(sys.argv))
